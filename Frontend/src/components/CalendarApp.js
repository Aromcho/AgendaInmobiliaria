...
function App({ onLogout }) {
  ...
  // ---- carga inicial del tablero de tareas (3 columnas fijas del backend) ----
  useEffect(() => {
    let active = true;
    fetchCollection('tasks').then((remote) => {
  ...
  const onPrev = () => setCursor((c) => view === "week" ? C.addDays(c, -7) : view === "day" ? C.addDays(c, -1) : C.addMonths(c, -1));
  const onNext = () => setCursor((c) => view === "week" ? C.addDays(c, 7) : view === "day" ? C.addDays(c, 1) : C.addMonths(c, 1));
  ...
  // ---- tablero de tareas ----
  const addTaskCard = async (listId, text) => {
    const created = await createResource('tasks', { listId, text });
    if (!created) return;
  ...
  const moveTaskCard = async (next, updates) => {
    setTaskLists(next);

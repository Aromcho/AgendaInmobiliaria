...
/* Seguimiento de clientes -> cartelera de leads del equipo (tarjetas + buscador + detalle editable) */
  ...
  // Normaliza "ofreci" a lista: soporta tanto el array nuevo como el texto suelto viejo
  function toList(v) {
  ...
  function ClientCard({ item, index, onOpen, onDelete, onColor, onEmoji }) {
    ...
    const ofrecidos = toList(item.ofreci);
      ...
      e('div', { className: 'cl-body' },
        ...
        ofrecidos.length
          ? e('div', { className: 'cl-offer-list' },
              ofrecidos.slice(0, 3).map((t, i) => e('div', { className: 'cl-line', key: i }, e('span', { className: 'cl-line-ico' }, '🏠'), e('span', { className: 'cl-line-txt' }, t))),
              ofrecidos.length > 3 ? e('div', { className: 'cl-line cl-more' }, `+${ofrecidos.length - 3} más`) : null,
            )
          : null,
        (!item.busca && !ofrecidos.length) ? e('div', { className: 'cl-line cl-line-empty' }, 'Sin detalles todavía… (click para completar)') : null,
      ),
  ...
  function ClientesView({ items: allItems, query, agentFilter, onOpen, onDelete, onColor, onEmoji, onNew }) {
    const list = useMemo(() => {
      ...
      return (allItems || []).filter((it) => {
        ...
        if (q) {
          const hay = [it.cliente, it.telefono, it.email, it.busca, ...toList(it.ofreci)].filter(Boolean).join(' ').toLowerCase();
  ...
  function OfferedListItem({ value, onChange, onRemove }) {
    ...
    return e('div', { className: 'cd-list-item' },
      ...
      editing
        ? e('input', {
            autoFocus: true, value: val, onChange: (ev) => setVal(ev.target.value), onBlur: commit,
            onKeyDown: (ev) => { if (ev.key === 'Enter') { ev.preventDefault(); commit(); } if (ev.key === 'Escape') { setVal(value); setEditing(false); } },
          })
        : e('span', { className: 'cd-list-text', onClick: () => setEditing(true) }, value),
    ...
    return e('div', { className: 'detail-chip cd-chip cd-list-chip full' },
      ...
      e('div', { className: 'dc-text' },
        ...
        items.length
          ...
          : e('div', { className: 'cd-placeholder' }, 'Sin propiedades ofrecidas todavía'),
        adding
          ? e('input', {
              className: 'cd-list-input', autoFocus: true, value: draft, placeholder: 'Ej. depto en Av. San Martín 123',
              onChange: (ev) => setDraft(ev.target.value), onBlur: addDraft,
              onKeyDown: (ev) => { if (ev.key === 'Enter') { ev.preventDefault(); addDraft(); } if (ev.key === 'Escape') { setDraft(''); setAdding(false); } },
            })
          : e('button', { type: 'button', className: 'cd-list-add', onClick: () => setAdding(true) }, e(I.Plus, { width: 12, height: 12 }), 'Agregar propiedad'),
      ),
    );
  }
  ...
  function OfferFormRows({ value, onChange }) {
    ...
    return e('div', { className: 'fg-offer-list' },
      value.map((t, i) => e('div', { className: 'fg-offer-row', key: i },
        e('span', { className: 'fg-offer-txt' }, t),
        e('button', { type: 'button', className: 'fg-offer-del', title: 'Quitar', onClick: () => removeAt(i) }, e(I.Close, { width: 12, height: 12 })))),
      e('div', { className: 'fg-offer-add' },
        e('input', {
          value: draft, placeholder: 'Ej. depto en Av. San Martín 123', onChange: (ev) => setDraft(ev.target.value),
          onKeyDown: (ev) => { if (ev.key === 'Enter') { ev.preventDefault(); add(); } },
        }),
        e('button', { type: 'button', className: 'btn ghost sm', onClick: add }, e(I.Plus, { width: 14, height: 14 }), 'Agregar')),
    );
  }
